"""Run the authoritative full-history backtest for MES RSI trend pullback."""
...
from mes_rsi_trend_pullback_common import (
    FULL_START,
    METRICS_PATH,
    TRADE_LOG_PATH,
    format_metrics,
...
def main() -> None:
    ...
    checks = {
        ...
        "all_entries_adx_ge_20": all(float(trade.metadata.get("signal_adx_14", 0.0)) >= 20.0 for trade in result.trades),
        "all_entries_above_sma_100": all(float(trade.metadata.get("signal_close", 0.0)) > float(trade.metadata.get("signal_sma_100", 0.0)) for trade in result.trades),
        "all_entries_rsi_lt_25": all(float(trade.metadata.get("signal_rsi_2", 100.0)) < 25.0 for trade in result.trades),
    ...
    print("MES RSI Trend Pullback Backtest")
